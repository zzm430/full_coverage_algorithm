...
{
Controller::Controller() { holonomic_robot_enable = false; }
                                        ...
                                        const ros::Duration sample_time, tracking_pid::PidDebug* pid_debug)
{
    ...
    ROS_INFO_STREAM("the car length is :" << l);
    ...
    ROS_INFO_STREAM("the flag is : "<<track_base_link_enabled );
    track_base_link_enabled = true;
    ...
    Ki_long = 10;
    Kd_long = 0.1;
    ROS_INFO_STREAM("the kp ki kd is" << Kp_long   << " " << Ki_long << " " << Kd_long);
    ...
    {
        double scale_long_control_1min =
            (fabs(error_ang.at(0)) - dead_zone_yaw_error_cal) / (max_yaw_error_cal - dead_zone_yaw_error_cal);
